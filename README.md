...
1. **Ask the data** — natural-language questions answered by a server-side AI assistant using the read-only SQLite database.
...
3. Copy `.env.example` to `.env` and add your OpenAI API key and an available model name.
4. Run `npm start`, then open `http://localhost:3000`.

The prohibited-purchase dashboard works without a key. The natural-language tab requires `OPENAI_API_KEY`.
...
3. In the host's **environment variables / secrets** settings, add `OPENAI_API_KEY` and optionally `OPENAI_MODEL`. Do **not** commit `.env`.
4. Add your deployed URL and GitHub repository URL to Part IV of the assignment document.

## Security

`.env` is excluded from Git. The API key is read only by `server.js` on the server; it is never sent to the browser, written to source code, or displayed in the dashboard. The assistant is limited to a single read-only `SELECT` query against `pcards`.
